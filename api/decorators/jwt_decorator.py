    ...
    def wrapped_view(request, *args, **kwargs):
        ...
        if token_type == 'microsoft':
            try:
                ...
                return JsonResponse({"error": f"Token de microsoft expirado {str(e)}"}, status=status.HTTP_401_UNAUTHORIZED)
            except jwt.InvalidTokenError as e:
                return JsonResponse({"error": f"Token de Microsoft inválido: {str(e)}"}, status=status.HTTP_401_UNAUTHORIZED)
            except Exception as e:
                return JsonResponse({"error": f"Error al validar el token de Microsoft: {str(e)}"}, status=status.HTTP_401_UNAUTHORIZED)
            ...
            try:
                ...
                return JsonResponse({"error": "Token propio expirado"}, status=status.HTTP_401_UNAUTHORIZED)
            except jwt.InvalidTokenError as e:
                return JsonResponse({"error": f"Token propio inválido: {str(e)}"}, status=status.HTTP_401_UNAUTHORIZED)
            except Exception as e:
                return JsonResponse({"error": f"Error al validar el token propio: {str(e)}"}, status=status.HTTP_401_UNAUTHORIZED)
        else:
